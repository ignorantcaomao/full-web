...
def get_workitems():
    ...
    # cursor = connection.cursor()
        ...
        # with DbOperate('../hello.sqlite3') as db:
        #     sql_result = db.execute_sql("select count(*) from taskTime_workitems")
        #     # 获取表里的数据
        #     counts = next(sql_result)[0]
        #     print(counts)
        result = []
        ...
        # with DbOperate('../hello.sqlite3') as db:
        #     db.executemany_sql(
        #         "insert into taskTime_workitems (duration, created, text, author, id, idReadable) values (?, ?, ?, ?, ?, ?)",
        #         result
        #     )
...
def get_jszc_info():
    ...
    # with DbOperate('../hello.sqlite3') as db:
    #     sql_result = db.execute_sql("select count(*) from taskTime_jszc_info")
    #     # 获取表里的数据
    #     counts = next(sql_result)[0]
    ...
    # with DbOperate('../hello.sqlite3') as db:
    #     db.executemany_sql(
    #         "insert into taskTime_jszc_info (id, projectId, taskName, projectName, department, author, created, updated) values (?, ?, ?, ?, ?, ?, ?, ?)",
    #         data_list
    #     )
